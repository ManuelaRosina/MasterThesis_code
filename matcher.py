...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets, positive_map=None):
        ...
        if positive_map is not None:
            ...
            tgt_bbox = torch.cat([v["scaled boxes"] for v in targets])
            ...
            #print(out_bbox.device) 
            #print(tgt_bbox.device)
            cost_bbox = torch.cdist(out_bbox, tgt_bbox, p=1)
            assert cost_class.shape == cost_bbox.shape
    
            # Compute the giou cost betwen boxes
            #print("bbox: ", outputs["pred_boxes"])
            #print(tgt_bbox)
            ...
            sizes = [len(v["scaled boxes"]) for v in targets]
            ...
            #	print("c[i]: ", c[i])
            #	print("lin_sum: ", linear_sum_assignment(c[i]))
            indices = [linear_sum_assignment(c[i]) for i, c in enumerate(C.split(sizes, -1))]
            return [(torch.as_tensor(i, dtype=torch.int64), torch.as_tensor(j, dtype=torch.int64)) for i, j in indices]
